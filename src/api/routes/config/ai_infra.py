...
from src.api.models.config import AIInfraConfig, AIInfraConfigUpdate, EndpointsList
...
def get_ai_infra_config(
    ...
    try:
        ...
        logger.error(f"Error getting AI infra config for profile {profile_id}: {e}", exc_info=True)
        raise HTTPException(
...
def update_ai_infra_config(
    ...
    try:
        ...
        logger.error(f"Error updating AI infra config for profile {profile_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
