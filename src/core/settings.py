...
Use src.settings.settings_db instead, which provides database-backed configuration.

This module is kept only for:
- Backward compatibility during migration
- Initializing default profile values (via init_default_profile.py)

New code should use: from src.settings.settings_db import get_settings
"""
...
# Pydantic schemas for configuration sections
...
def create_settings() -> AppSettings:
    """
    Create application settings by combining YAML settings and environment variables.
    ...
    try:
        ...
        # Create nested Pydantic schemas
...
def reload_settings() -> AppSettings:
    ...
    Useful during development when settings files change.
