...
# IMPORTANT: --ignore-scripts disables ALL package postinstall hooks. We
# do this for two reasons:
#   1. Playwright's postinstall downloads Chromium (~150 MB) which we
#      don't ship in the wheel — the deployed app's setup.sh runs
#      `playwright install chromium` at boot anyway.
#   2. More importantly, npm on GitHub Actions runners regularly crashes
#      mid-install with "npm error Exit handler never called!" leaving
#      node_modules with empty package directories. The crash typically
#      happens during postinstall scripts. v0.3.2 and v0.3.3 attempts
#      both shipped broken because of this. PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD
#      alone wasn't enough — npm still crashed somewhere else.
# cli.js, html2pptx.js, etc. are part of the playwright npm tarball
# (not generated by postinstall), so --ignore-scripts gives us a complete
# working node_modules without the flake.
export PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1

if [ -f package-lock.json ]; then
  npm ci --omit=dev --ignore-scripts
else
  echo "WARN: no package-lock.json — falling back to 'npm install --omit=dev --ignore-scripts'"
