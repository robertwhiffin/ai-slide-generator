/**
 * GenieDiscovery — inline panel for searching and selecting Genie spaces.
 *
 * Extracted from the original ToolPicker. Fetches all available Genie-type
 * tools, filters client-side, and delegates to GenieDetailPanel on selection.
 */

import React, { useEffect, useState, useRef } from 'react';
import { Loader2, RefreshCw, Search, X } from 'lucide-react';
import { api } from '../../../services/api';
import type { AvailableTool, GenieTool, ToolEntry } from '../../../types/agentConfig';

interface GenieDiscoveryProps {
  onSelect: (tool: GenieTool) => void;
  onPreview: (tool: AvailableTool) => void;
  onClose: () => void;
  existingTools: ToolEntry[];
}

function isGenieAlreadyAdded(candidate: AvailableTool, existing: ToolEntry[]): boolean {
  return existing.some(
    t => t.type === 'genie' && candidate.type === 'genie' && t.space_id === candidate.space_id,
  );
}

export const GenieDiscovery: React.FC<GenieDiscoveryProps> = ({
  onPreview,
  onClose,
  existingTools,
}) => {
      ...
      <div className="max-h-48 overflow-y-auto px-2 pb-2">
        ...
        {!loading &&
          ...
          filtered.map((tool, idx) => {
            const added = isGenieAlreadyAdded(tool, existingTools);
            return (
              <button
                key={`genie-${tool.space_id ?? idx}`}
                disabled={added}
                onClick={() => {
                  onPreview(tool);
                  onClose();
