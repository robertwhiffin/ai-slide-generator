...
def load_theme_from_json(path: Union[str, Path]) -> SlideTheme:
    """Load a SlideTheme from a JSON file.

    Recognized keys: background_rgb, font_family, title_font_size_px,
    subtitle_font_size_px, body_font_size_px, title_color_rgb,
    subtitle_color_rgb, body_color_rgb, primary_rgb, accent_rgb,
    header_bar_height_px, logo_url, footer_text.
    """
    p = Path(path).expanduser().resolve()
    data = json.loads(p.read_text(encoding="utf-8"))
    return SlideTheme(
        background_rgb=tuple(data.get("background_rgb", (255, 255, 255))),
        font_family=data.get("font_family", "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif"),
        title_font_size_px=int(data.get("title_font_size_px", 44)),
        subtitle_font_size_px=int(data.get("subtitle_font_size_px", 26)),
        body_font_size_px=int(data.get("body_font_size_px", 18)),
        title_color_rgb=tuple(data.get("title_color_rgb", (0, 0, 0))),
        subtitle_color_rgb=tuple(data.get("subtitle_color_rgb", (80, 80, 80))),
        body_color_rgb=tuple(data.get("body_color_rgb", (20, 20, 20))),
        primary_rgb=tuple(data.get("primary_rgb", (46, 46, 56))),
        accent_rgb=tuple(data.get("accent_rgb", (255, 102, 0))),
        header_bar_height_px=int(data.get("header_bar_height_px", 0)),
...
def create_basic_html(output_path: str | Path, *, title: str = "Hello, world!", subtitle: str = "Generated HTML deck") -> Path:
    destination = Path(output_path).expanduser().resolve()
    destination.parent.mkdir(parents=True, exist_ok=True)
    deck = HtmlDeck(theme=SlideTheme())
    deck.add_title_slide(title=title, subtitle=subtitle, authors=[], date="")
    html = deck.to_html()
