...
def load_slides():
    ...
    html_path = Path("/Users/robert.whiffin/Documents/slide-generator/output/databricks_benefits.html")
    html_str = html_path.read_text(encoding="utf-8")
    iframe = (
        '<div style="position:relative;width:100%;max-width:100%;aspect-ratio:16/9;">'
        f'  <iframe srcdoc="{html.escape(html_str)}" '
...
SYSTEM_PROMPT = """
You are a slide creation assistant. Users interact with you to create their slide decks with natural language. You have access to a set of tools that can update a HTML slide deck. There are 5 tools available to you;
...
chatbot_instance = chatbot.Chatbot(html_deck=html_deck, llm_endpoint_name=LLM_ENDPOINT_NAME)
...
def update_slides():
    ...
    if not current_html.strip():
        return load_slides()  # Fallback to demo slides if no content
