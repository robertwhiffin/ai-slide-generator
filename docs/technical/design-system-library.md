...
**A personal slide-style default can suppress org-default design-system seeding** — when the config already carries a `slide_style_id`, which is what that guard requires. It is then reached before the org-default branch and returns, so the org brand is not resolved on that surface at all. Any copy asserting that a personal style default cannot outrank the org design system is wrong; the behaviour is pinned by `frontend/tests/e2e/design-system-selector.spec.ts`.

**Clearing a personal design-system default releases the slot, it does not re-seed it.** `setUserDefaultDesignSystem(null)` writes `design_system_id: null`, `template_id: null`, `slide_style_id: userPreferredStyleId()` and `style_source: 'user'`, then drops the `localStorage` key. The write happens **before** the key is removed, so a failed release leaves the preference in place for the next attempt. The Clear control does not stay visible in the meantime: the caller drops its local `userDefaultId` state before awaiting the release (`DesignSystemLibrary.tsx:163`) and the button is rendered from that state, not from the key (`:327`). It therefore disappears on the failing render and returns only once the component remounts and re-reads the surviving key (`:46`). Workspace defaults reappear only when a later entry path performs seeding, because the surface now carries `style_source: 'user'`.
...
| `'seeded'` | **Not an early return.** The config clears both guards and continues into the remaining ones on equal footing with a fresh surface. With an empty design-system slot and no personal preference claiming it first, it reaches the org-default branch, which takes the slot, clears the seeded style and re-stamps `'seeded'`. |
