...
| LLM judge / Genie: `storage.cloud.databricks.com` + `Connection refused` | **Verification:** allowlist egress, use Admin **Direct** judge, or rely on Tellr’s **direct judge fallback** after evaluate fails ([`llm_judge.py`](../../src/services/evaluation/llm_judge.py)). **Chat / Genie:** with Admin **Direct**, Tellr skips MLflow ``start_span`` around slide generation by default (see ``src/core/mlflow_agent_spans.py``). If spans are forced on (``TELLR_MLFLOW_DISABLE_AGENT_SPANS=0``) or LangChain autolog is enabled, trace uploads may still require egress. |
