...
| LLM judge: `storage.cloud.databricks.com` + `RESOURCE_DOES_NOT_EXIST` | `mlflow.genai.evaluate` still touches regional artifact storage for some trace paths. Allowlist egress, set **Admin → Judge → Direct** to skip MLflow for verification, or rely on Tellr’s **automatic direct judge fallback** in [`llm_judge.py`](../../src/services/evaluation/llm_judge.py) when evaluate fails for infra reasons (verification still returns a rating). |
