...
- **Environment:** Requires `DATABRICKS_HOST` for MLflow tracking. On a Databricks Apps deployment MLflow needs no token of its own — it delegates to the SDK credential chain (`MLFLOW_ENABLE_DB_SDK` defaults on), which resolves the platform-injected OAuth M2M credentials. Locally, a `DATABRICKS_TOKEN` in `.env` still works (SDR-4437 F-CR-18).
