...
**Key management:**
- **Production:** Set `GOOGLE_OAUTH_ENCRYPTION_KEY` environment variable (base64-encoded 32-byte key generated by `Fernet.generate_key()`).
...
|----------|----------|-------------|
| `GOOGLE_OAUTH_ENCRYPTION_KEY` | Production | Fernet key for encrypting credentials/tokens |
