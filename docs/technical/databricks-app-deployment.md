...
env:
  ...
  - name: GOOGLE_OAUTH_ENCRYPTION_KEY
    value: "<auto-generated Fernet key>"
  - name: LAKEBASE_TYPE
...
The `encryption_key` parameter provides a Fernet key used to encrypt Google OAuth
credentials stored in the database. Behavior:

- **create()**: auto-generates a key if not provided
- **update()**: reads the existing key from the deployed `app.yaml` if not provided,
