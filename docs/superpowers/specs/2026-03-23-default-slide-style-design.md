...
This is distinct from `is_system` (which means "protected from edit/delete"). Any style — system or user-created — can be the default. Exactly one style has `is_default=True` at any time, enforced by the API layer.

### Migration

In `_run_migrations()` in `src/core/database.py`:

```sql
-- Add column (idempotent)
ALTER TABLE slide_style_library ADD COLUMN IF NOT EXISTS is_default BOOLEAN DEFAULT FALSE NOT NULL;
...
Idempotent: the `UPDATE` only runs when no default exists, so redeploys don't overwrite a user's choice.
...
- `GET /api/settings/slide-styles` — already returns all model fields; `is_default` is included automatically. Add `is_default` to the `SlideStyleResponse` pydantic schema.
- Create/Update/Delete — no changes. `is_default` is managed only through the dedicated endpoint.
...
On first mount with no stored `pendingAgentConfig`, after loading the default profile's config, check `userDefaultSlideStyleId` from localStorage. If set, override `slide_style_id`.
...
- Unit tests for `POST /api/settings/slide-styles/{id}/set-default`: sets default, unsets previous, rejects inactive style, rejects nonexistent style
