...
def _write_requirements(
    ...
    # No resolver-speedup pins here: the databricks-tellr-app wheel ships a fully
    # pinned dependency closure in its own metadata (see its pyproject.toml), so a
    # bare install resolves without backtracking. This keeps the pins in a single
    # source of truth (the wheel) that both the PyPI-install and local-wheel paths
    # share.
    content = "\n".join(
        [
